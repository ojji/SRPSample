...
namespace EcommerceLib.Tests.PricingStrategyTests.BuyXItemsGetYPercentOffTests
{
    public class BuyXItemsGetYPercentOffTests
    {
        [Test]
        public void Empty_or_null_itemid_should_throw_ArgumentNullException()
        {
            Assert.DoesNotThrow(() => new BuyXItemsGetYPercentOff("item-1", 1, 0.1m));
            Assert.Throws<ArgumentNullException>(() => new BuyXItemsGetYPercentOff("", 1, 0.1m));
            Assert.Throws<ArgumentNullException>(() => new BuyXItemsGetYPercentOff(null, 1, 0.1m));
        ...
        {
            Assert.DoesNotThrow(() => new BuyXItemsGetYPercentOff("item-1", 1, 0.1m));
            Assert.Throws<ArgumentOutOfRangeException>(() => new BuyXItemsGetYPercentOff("item-1", 0, 0.1m));
            Assert.Throws<ArgumentOutOfRangeException>(() => new BuyXItemsGetYPercentOff("item-1", -1, 0.1m));
        }

        [Test]
        public void Discount_should_be_between_0_and_100_percent()
        {
            Assert.DoesNotThrow(() => new BuyXItemsGetYPercentOff("item-1", 4, 0.2m));
            Assert.Throws<ArgumentOutOfRangeException>(() => new BuyXItemsGetYPercentOff("item-1", 4, 1.00m));
            Assert.Throws<ArgumentOutOfRangeException>(() => new BuyXItemsGetYPercentOff("item-1", 4, 0.00m));
        }
